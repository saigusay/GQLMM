...
#-----------------------------------------------------------------------------------------------------#
#   Simulation studies for Poisson response variable                                                  #
#-----------------------------------------------------------------------------------------------------#
...
### Note:
  ...
  #        effect parameter under the Poisson quasi-linear mixed model; and
  #     2) the proposed conditional AIC for the Poisson quasi-linear mixed model
...
#-----------------------------------------------------------------------------------------------------
#   Function to estimate the regression parameters														  
#-----------------------------------------------------------------------------------------------------
...
sim_pois = function(ni,tau.true,be.true,b.dist,b.para,b.vary,var.x,var.z,Tau,n.sim,iter.step,iter.cal){
...
set.seed(it)
	...
	if(tau.true==0){
		lam.true = exp(x%*%be.true+apply(z*bb.true,1,sum))
	}else{
		lam.true = exp( 2/tau.true*log( 1/2*exp(tau.true*x%*%be.true)+1/2*exp(tau.true*apply(z*bb.true,1,sum))) )
	}
	y = matrix(rpois(length(lam.true),lam.true),ncol=1)
	out.glmer = glmer.pois(y,x,z,id)
	...
	for(tau in Tau){
		...
		while((!is.nan(dif) & dif>sdif) & ie<iter.step){
			...
			A=beb.est.pois(tau,be,b,th,y,x,z,dv)
			if(class(A)[1]!="try-error"){
				be=A$be; b=A$b
			}
			B=try(d.est.pois(tau,be,b,th,y,x,z,dv),silent=T)
		...
		ci = caic.pois(tau,be,b,th,y,x,z,dv)
...
var.x = rep(1,length(be.true)-1)
var.z = rep(1,length(b.para)-1)
...
sim_pois(ni,tau.true,be.true,b.dist,b.para,b.vary,var.x,var.z,Tau,n.sim,iter.step,iter.cal)
